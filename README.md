...
Твой проект будет зависеть от внешних библиотек (express и прочие). Если ты склонировал репозиторий и в нём уже есть файл `package.json`, то чтобы начать работу, тебе необходимо устновить зависимости. Для этого в папке проекта следует выполнить `npm install`.
...
```
*Рисунок 8*. Фрагмент кода [React-компонента](https://ru.reactjs.org/).

Когда мы назначаем переменную в блоке, эта переменная приходит в React-компонент в props.
...
Мы будем использовать форму для отправки названия подбадривающего приветствия. В `app/views/Main.jsx` мы определяем форму, а также мы даем форму некоторым атрибутам. У формы есть `метод`, который представлен посредством `post` и `action`, который установлен на `"/cheers"`. Эти атрибуты объединяются, чтобы определить, что происходит при отправке формы. Отправка этой формы подскажет браузеру сделать запрос `POST` на путь `/cheers`.
